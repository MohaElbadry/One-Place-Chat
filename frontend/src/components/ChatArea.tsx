...
export default function ChatArea({
      ...
      if (result.success) {
        ...
        const assistantMessage: Message = {
          ...
          clarificationRequest:
            result.data.assistantResponse.clarificationRequest,
        };
    ...
    return (
      <div className="mt-3 p-3 bg-blue-50 border-l-4 border-blue-400 rounded-r-lg">
        <div className="text-sm font-medium text-blue-800">
          <p className=" inline text-base font-bold text-primary">
            {" "}
            Tool Detected :{" "}
          </p>{" "}
          {toolMatch.tool.name}
  ...
  const renderClarificationRequest = useCallback(
    (clarificationRequest: any) => {
            ...
            {clarificationRequest.missingFields && (
              <div className="mt-2 text-xs">
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {clarificationRequest.missingFields.map(
  ...
  const markdownComponents = useMemo(
    () => ({
      ...
      h1: ({ children }: any) => (
        <h1 className="text-xl font-bold mb-3 text-foreground break-words">
          {children}
        </h1>
      ),
      h2: ({ children }: any) => (
        <h2 className="text-lg font-bold mb-2 text-foreground break-words">
          {children}
        </h2>
      ),
      h3: ({ children }: any) => (
        <h3 className="text-md font-semibold mb-2 text-foreground break-words">
          {children}
        </h3>
      ),
      ul: ({ children }: any) => (
        <ul className="list-disc ml-6 mb-3 space-y-1 break-words">
          {children}
        </ul>
      ),
      ol: ({ children }: any) => (
        <ol className="list-decimal ml-6 mb-3 space-y-1 break-words">
          {children}
        </ol>
      ),
      li: ({ children }: any) => (
        <li className="text-foreground/90 break-words">{children}</li>
      ),
  ...
  const renderedMessages = useMemo(() => {
    return messages.map((message) => {
      ...
      const { content: displayContent, isTruncated } = truncateMessage(
        message.content,
        message.id
      );
      ...
      return (
        ...
        >
          <div
            className={`max-w-[80%] min-w-0 ${isUser ? "order-2" : "order-1"}`}
          >
            {/* Message bubble */}
            <div
            ...
            >
              ...
              <div
                className={`text-xs mb-3 font-medium ${
                  isUser ? "text-info" : "text-muted-foreground"
                }`}
              >
                <span className="font-semibold text-muted-foreground font-poppins">
              ...
              {isLongMessage(message.content) && (
                <Button
                  ...
                  className={`mt-3 h-auto px-3 py-1.5 ${
                    isUser
            ...
            {isAssistant && (
              <div className="mt-2">
                ...
                {message.clarificationRequest &&
                  renderClarificationRequest(message.clarificationRequest)}
              </div>
            )}
          </div>
        </div>
        ...
        {messages.length === 0 && !currentConversationId ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <div className="text-2xl font-display text-primary mb-3">
                Start a new conversation
              </div>
              <div className="text-base text-muted-foreground font-medium font-poppins">
                ...
                <div className="flex-shrink-0 px-4 pt-4 pb-2 border-border/60">
                  <div className="mb-3 flex flex-col gap-2 ">
                    <Button
                      variant="outline"
                      onClick={() =>
                        setInputValue("Get a JSON response from an API")
                      }
                    ...
                    <Button
                      variant="outline"
                      onClick={() =>
                        setInputValue("Find pets by status available")
                      }
              ...
              <div className="bg-warm-gray border border-border/60 rounded-2xl rounded-bl-sm px-5 py-4 shadow-soft">
                <div className="flex items-center gap-3 text-muted-foreground">
                  <div className="flex space-x-1">
                    ...
                    ></div>
                    <div
                      className="w-2.5 h-2.5 bg-secondary rounded-full animate-bounce"
            ...
            {uploadStatus === "idle" && (
              <div className="border-2 border-dashed border-border rounded-2xl p-8 text-center hover:border-primary/50 transition-colors">
                ...
                <p className="text-muted-foreground mb-4">
                  Upload a JSON file containing your OpenAPI/Swagger
                  specification to generate tools
                </p>
                ...
                <p className="text-xs text-muted-foreground mt-3">
                  Supported formats: JSON files with OpenAPI 3.0 or Swagger 2.0
                  specifications
                </p>
              </div>
            )}
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700">
                      Generating tools and embeddings
                    </p>
                  </div>
                </div>
              ...
              <div className="bg-green-50 border border-green-200 rounded-2xl p-6 mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700">
                      Tools have been generated and stored
                    </p>
                  </div>
                </div>
              </div>
