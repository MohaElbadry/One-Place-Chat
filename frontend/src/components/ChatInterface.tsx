...
export default function ChatInterface() {
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      {/* Tool Details Modal */}
      <Dialog open={showToolModal && selectedTool !== null} onOpenChange={setShowToolModal}>
        <DialogContent className="sm:max-w-2xl max-h-[90vh] bg-white">
          {selectedTool && (
            <ScrollArea className="max-h-[calc(90vh-8rem)]">
              <DialogHeader className="mb-6">
                <DialogTitle className="text-2xl font-bold text-gray-900">Tool Details</DialogTitle>
              </DialogHeader>
              ...
              <div className="bg-gradient-to-r bg-gray-50 from-primary/10 to-accent/10 rounded-2xl p-6 mb-6 border border-primary/20">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-xl font-display font-bold text-gray-600 mb-1">{selectedTool.name}</h4>
                    <div className="flex items-center gap-2">
                      <Badge className={`${selectedTool.method === 'GET' ? 'bg-green-500 hover:bg-green-600' :
                        selectedTool.method === 'POST' ? 'bg-blue-500 hover:bg-blue-600' :
                          selectedTool.method === 'PUT' ? 'bg-yellow-500 hover:bg-yellow-600' :
                            selectedTool.method === 'DELETE' ? 'bg-red-500 hover:bg-red-600' :
                              selectedTool.method === 'PATCH' ? 'bg-purple-500 hover:bg-purple-600' : 'bg-gray-500'
                        }`}>
                        {selectedTool.method}
                      </Badge>
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={selectedTool.deprecated ? "destructive" : "outline"} className={!selectedTool.deprecated ? "bg-green-50 text-green-700 border-green-200" : ""}>
                          {selectedTool.deprecated ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Read Only</span>
                        <Badge variant="outline" className={selectedTool.readOnly ? "bg-blue-50 text-blue-700 border-blue-200" : "bg-gray-50 text-gray-700 border-gray-200"}>
                          {selectedTool.readOnly ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Open World</span>
                        <Badge variant="outline" className={selectedTool.openWorld ? "bg-purple-50 text-purple-700 border-purple-200" : "bg-gray-50 text-gray-700 border-gray-200"}>
