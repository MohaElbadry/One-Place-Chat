...
/**
 * Simple Embedding Test Script (JavaScript Version)
 * 
...
async function testEmbeddings() {
  ...
  try {
    ...
    const toolsCollection = await chromaClient.getOrCreateCollection({
      name: 'test_tools_simple',
      metadata: { description: 'Test tools for embedding demo (simple)' }
    });
    ...
    for (let i = 0; i < sampleTools.length; i++) {
      ...
      const toolId = `test_tool_simple_${i + 1}`;
      toolIds.push(toolId);
      ...
      toolMetadatas.push({
        ...
        method: tool.method,
        path: tool.path,
        tags: tool.tags.join(','), // Convert array to comma-separated string
    ...
    for (const useCase of useCases) {
      ...
      try {
        ...
        console.error(`   ❌ Use case test failed:`, error.message);
      }
    }
